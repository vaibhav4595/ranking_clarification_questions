...
def batch_iter(ids, post_content, qa_dict, vocab, batch_size, shuffle=False):
    ...
    for idx in shuffled_ids:
        ...
        post = post_content[idx] # vocab.words2indices(post_content[idx])
        ...
        for i, (question, answer) in enumerate(zip(qa_dict[idx][0], qa_dict[idx][1])):
            ...
            p = " ".join([x for x in post ])
            q = " ".join([x for x in question ])
            a = " ".join([x for x in answer ])

            #print(post_content[idx], qa_dict[idx][0], qa_dict[idx][1])

            post_words.append(p) # post
            question_words.append(q) #  (vocab.words2indices(question))
            answer_words.append(a)   #(vocab.words2indices(answer))
