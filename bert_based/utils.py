...
def bert_batch_iter(ids, post_content, qa_dict, batch_size, shuffle=False):
    ...
    for idx in shuffled_ids:
        for i, (question, answer) in enumerate(zip(qa_dict[idx][0], qa_dict[idx][1])):
            ...
            example_ids.append(idx + '_' + str(i + 1))
            post_words.append(post_content[idx])
...
def triplet_batch_iter(ids, post_content, qa_dict, batch_size, shuffle=False):
    ...
    for idx in shuffled_ids:
        ...
        truth_question, truth_answer = qa_dict[idx][0][0], qa_dict[idx][1][0]
        example_ids.append(idx + '_' + str(1))
        post_words.append(post_content[idx])
        question_words.append(truth_question)
        answer_words.append(truth_answer)
        labels.append(1)

        choice = random.choice([i for i in range(1, 10)])
        neg_question, neg_answer = qa_dict[idx][0][choice], qa_dict[idx][1][choice]
        example_ids.append(idx + '_' + str(2))
        post_words.append(post_content[idx])
        question_words.append(neg_question)
        answer_words.append(neg_answer)
        labels.append(0)
