...
import time
import utils
from bert_critic_pq import BertRank
...
def train():
    ...
    for ep in range(args.max_epochs):
        ...
        for batch in utils.triplet_batch_iter(train_ids, post_content, qa_dict, args.batch_size):
            ...
            for lab in labels:
                if lab == 0:
                    mult_tensor.append(-1)
                else:
                    mult_tensor.append(1)

            outputs = model(ids, posts, questions)
            outputs = outputs.squeeze(1)
            mult_tensor = torch.tensor(mult_tensor).to(device=device)

            loss = torch.mean(outputs * mult_tensor)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_dir", type=str, default="../models/bert_critic_pq/distil/")
